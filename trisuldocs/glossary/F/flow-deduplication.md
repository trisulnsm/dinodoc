...
Trisul supports flow‑deduplication through **flow‑leg correlation** and the **MergeMultipleSources** configuration, letting operators normalize traffic visibility across overlapping exporters while preserving underlying telemetry. By default, Trisul keeps each flow leg as a separate record, so you can drill down from a router interface to its flows. Turn on the **Flow Legs Correlation** option in WebTrisul to group the legs in the flow tools, or set `MergeMultipleSources` in the NetFlow configuration to merge them into one flow. This is useful in ISP, multi‑site enterprise, and datacenter environments where the same flows are visible at multiple vantage points. Trisul combines this with **historical traffic analysis**, **Explore Flows**, **Flow Taggers**, and **Interface Tracking** so that operators can move between clean traffic‑summaries and raw, multi‑source telemetry as needed.
