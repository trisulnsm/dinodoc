h1. Step by step - working with resources
...
# "Step 1":#step_1_:_find_matching_resources - Retrieve matching resource IDs
# "Step 2":#step_2_:_find_matching_resources - Print attributes of each matching resource
# "Step 3":#step_3_:_find_matching_resources - Show readable IP addresses and port numbers 
# "Step 4":#step_4_:_find_matching_resources - Show host names and port lables 

h2. Step 1 : Find matching resources

Retrieve the Resource IDs of all matching resources. A resource id is a tuple of (slice_id,resource_id) as defined "here":/docs/ref/trpproto.html

The message we will use is "ResourceGroupRequest":/docs/ref/trpproto.html

h3. Code

h4. What this does 

# Boiler plate - includes modules from the @trisulrp@ gem (see "here":http://rubydoc.info/gems/trisulrp/frames for rdoc)
# Opens a connection to the Trisul host and port with the private key
# Gets the recent 6 hour time window available
# Constructs a ResourceGroupRequest for RG_URL (you can use RG_DNS if you want to see domain names that were logged by Trisul)
# We restrict maxitems to 5 for this tutorial
# Gets a response and prints a list of ResourceIDs
...
h2.  Step 2 : Print details of each resource 

The previous step gave us five resource ids that matched our query. Now, we need to print out the attributes of each of those 5 resources. The message we need to use for that is "ResourceItemRequest":/docs/ref/trpproto.html
...
# IP Addresses look like "C0.A8.01.67, 34.3F.CD.FC.."
# Port Numbers look like "p-A9B2, p-0050.."
...
h2. Step 3 : Show keys in readable format
