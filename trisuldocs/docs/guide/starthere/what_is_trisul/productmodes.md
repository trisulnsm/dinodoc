...
| --- | --- | --- | --- |
| **Trisul NSM** ![Trisul NSM](image/nsm_prod.png) | Packet Capture Mode | Captures packets from a SPAN/port mirror, stores raw PCAPs, and generates rich metadata such as flows, applications, hosts, files, and alerts. | Deep troubleshooting, security visibility, investigation, and forensics. |
...
| **Trisul IPDR DoT Compliance Solution** ![Trisul IPDR](image/ipdr_prod.png) | Flow-Based Mode | Extracts subscriber activity records (IPDR) from live traffic in the required format for lawful retention. [**See Usecases**](https://trisul.org/trisul-ipdr-use-cases/) | ISPs that need DoT-compliant IPDR generation and long-term storage. |
...
See [**Processing Modes**](/docs/guide/starthere/what_is_trisul/dataflow#input-processing-modes) to learn how network data enters and is processed by Trisul.

:::note
A Product Mode is a preconfigured starting point for the selected use case. You can still configure Trisul further for your requirements.
:::
