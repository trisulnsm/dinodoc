...
Here we can see  we are using **6.2 MegaBytes** of disk per day using the [Storage Status](/docs/guide/ag/admintasks/storage_status) tool
...
If packet capture data needs to live on a different disk or mount point, relocate it using: [`trisulctl_probe relocate context`](/docs/guide/ag/basictasks/reloc). This is commonly done when packet capture volumes grow large.
