...
The DR Traffic Chart displays network traffic patterns that run in parallel with the primary site's traffic, ensuring continuous availability. Located below the DR Traffic Chart, DR Database slices serve as backups of the primary database, mirroring the same configuration as the Database Slices in [DB Status](/docs/guide/ag/admintasks/dbstatus#database-slices).
