...
Hosts with Email traffic

## Counter group

| GUID                                     | TYPE   | Bucke Size (secs) | Topper Bucket(Secs): |
| ---------------------------------------- | ------ | ----------------- | --------------------- |
| `{22D4082E-B8BA-40D0-A287-1F524DF8DA7B}` | Native | 60                 | 300                    |

## Meter list

| ID  | DESCRIPTION       | TOP COUNT | BOTTOM COUNT | TYPE                | UNITS   | DESCRIPTION |
| --- | ------------------- | --------- | ------------ | ------------------- | ------- |-------------|
| 0   | Total                  | 50        | 0            | VT_RATE_COUNTER     | Bps     | This meter reports the total bandwidth seen by this host in both the transmit and receive direction, for hosts observed carrying email traffic. |
...
| 14  | Into Interface         | 20        | 0            | VT_RATE_COUNTER     | Bps     | This meter appears only in a filtered counter group where the parent counter group is Email Hosts and the filter is a NetFlow interface. When used with the NetFlow Interface Tracker, it reports bandwidth involving this IP address as source or destination that is ingressing the interface specified in the interface tracker. |
| 15  | Outof Interface        | 20        | 0            | VT_RATE_COUNTER     | Bps     | This meter appears only in a filtered counter group where the parent counter group is Email Hosts and the filter is a NetFlow interface. When used with the NetFlow Interface Tracker, it reports bandwidth involving this IP address as source or destination that is egressing the interface specified in the interface tracker. |
