...
A search criteria form opens up where Multiple search criteria are 'AND'ed together to generate a search result for the targeted network flow data and can be toggled on or off using the [*Hide/ Show Search Form*](/docs/guide/ug/ui/elements#hide-show-search-form) option.
...
- The [*Toggle Labels*](/docs/guide/ug/ui/elements#toggle-labels) button enables the display of resolved names for the tuple parameters, providing an alternative view of the data. 
- The [*Download*](/docs/guide/ug/ui/elements#download-button) button allows for the export of the resulting data in a suitable format, facilitating further analysis and reporting.
- Each field in the tuple tab is [*Column Sortable*](/docs/guide/ug/ui/elements#column-sorter), enabling flexible data analysis and visualization.
