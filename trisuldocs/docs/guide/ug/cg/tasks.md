...
1. Click on the tag on the right side next to any host to open the [Key Dashboard](/docs/guide/ug/ui/key_dashboard)
...
- Find [Flow Tracker](/docs/guide/ug/cg/retrotools#flow-trackers) tool in retro dashboard
...
1. Follow instructions in [Using Search](/docs/guide/ug/ui/elements#using-search-tool)
2. And click **Flows** in Real Time stabbers in [Key
   Details](/docs/guide/ug/ui/key_dashboard#key-details) modules

See [Real Time Stabber](/docs/guide/ug/cg/stabber#real-time-stabber-flow-activity)

### Store only 1MB worth of packets of each flow

- Open [Trisul
  Configuration](/docs/guide/ref/trisulconfig) file as root

- Edit `<Rule mode="FLOWCAP1M"></Rule>`

- Read [Tuning packet storage](/docs/guide/ug/caps/packetstorage) for
  more examples
...
Click [Keyset Counter Group](/docs/guide/ag/context/keyset_countergroups) for more.
