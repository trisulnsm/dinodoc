...
|authorize probe |authorize a new probe - no need of private key (stays with probe)|install probe <probe-certificate>|
|authorize hub|authorize a hub identified by the hub-cert to connect|authorize hub hub-certificate-file|
|uninstall hub|install a hub (also need private key *.cert_secret) |uninstall hub <domain-id> <hub-id>|



<script type="text/javascript">
  document.addEventListener('DOMContentLoaded', function() {
    $('table tbody tr td:first-child').wrapInner("<code>")
  }, false);
</script>