h1. trisul_reencrypt : Reencrypt PCAP files 

Trisul stores all its PCAP files in an encrypted format. It uses a stream encryption cipher AES-128-CTR.  Using @trisul_reencrypt@ you can change the passphrase and reencrypt all files with the new passphrase. 
