...
| rawbytes     |  | a "Buffer":obj_buffer.html object | Raw bytes |
| total_bytes  |  | number  | Total number bytes in the packet, i.e. same as Packet:wire_length() |
| packet       |  | a "Packet":obj_packet.html object | The full packet  |
| testbit | bitno | bool | Test if a bit is set in this layer. Bits are numbered serially from _0 to 8*layer_bytes-1_ This is a shortcut for @layer:rawbytes():hval_8(bitno/8)@ <br/>You are encouraged to use @testbit@ because it increases performance by reducing Lua-C round trips. |
