...
| T.args | The `-args` command line option from [trisul](/docs/guide/programs/trisulprogram)|
...
| get_config        | `string` – ‘xml path’. See description column | `string` – value of config parameter  | Read a configuration parameter from the [Trisul Probe configuration file](/docs/guide/ref/trisulconfig). The XML Path supported is a very simple format “Node>Node>..Node”. So to read the config parameter *User* under *App* parent node the path is `App>User` |
