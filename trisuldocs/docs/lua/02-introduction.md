...
Since Trisul is a stream processor your scripts need to complete within a particular time budget. The scripts attached to the fast path need to complete much faster than those at the slow path. For scripts that involve I/O or someother time consuming task, an [asynchronous execution API](https://trisul.org/docs/lua/async.html) is available.
...
A script is attached either to the [frontend or the backend pipeline](https://trisul.org/docs/lua/basics.html#stream_processing) It cannot be attached to both. You do not have to do anything special to mark your script as attaching to frontend or backend. The “type of script” you write is automatically detected and attached to the correct place. We will shortly see what types of scripts are available.
...
For [development and test purposes](/docs/lua/debugger.html#development_mode) you can set the `TRISUL_LUA_PATHS` environment variable to add your own search paths to the probe nodes.
...
Using the `trisulctl_probe list lua` tool [see [trisulctl_probe](https://trisul.org/docs/ref/trisulctl_probe.html#app)] you can check if your LUA script has been discovered. If your script is not listed ensure that it is present in the specified directory and has the right permissions.
...
The best way is to use the `trisulctl_probe testbench` command to develop your scripts. See [Development Environment](https://trisul.org/docs/lua/debugger.html)

Alternately, you can manually convert the probe pipelines into single threaded by editing [trisulProbeConfig.xml](https://trisul.org/docs/ref/trisulconfig.html)
...
Also see: [Print and Logging](https://trisul.org/docs/lua/basics.html#print_and_logging)
...
The [ID Block](https://trisul.org/docs/lua/basics.html#id_block) a must for all plugins

**onload/onunload**

called when framework [loads or unloads](https://trisul.org/docs/lua/basics.html#onload_and_onload) the script

**message_subscriptions**

which message IDs do you want to listen to (see [Inter script communications](https://trisul.org/docs/lua/basics.html#inter_script_communication) )

**onmessage**

when some other plugin sends a message for you or broadcast (see [Inter script communications](https://trisul.org/docs/lua/basics.html#inter_script_communication) )

**request_async_workers**

optional number of workers you want to handle the async execution in this plugin. (see [T.async](https://trisul.org/docs/lua/async.html) )

**plugin type (simplecounter)**

a Table which contains your script of a particular type, simplecounter, reassembly handler etc (see [Script types](https://trisul.org/docs/lua/selector.html) and [Script selector cheatsheet](https://trisul.org/docs/lua/selector.html))

## On GUIDs

Trisul uses globally unique identifiers called GUIDs to identify each counter group, protocol, resource type, and many other things. Here is a list of [Common GUIDs](https://trisul.org/docs/ref/guid.html). When you want to create a new *type* of entity you need to assign a new GUID to refer to that. You can login as *admin* into Trisul to see list of all counter guids, resource guids, Full Text (FTS) guids, etc.
...
or when using the [Development Testbench](https://trisul.org/docs/lua/debugger.html#development_mode) – the print(..) messages send output to the terminal as they normally do
...
Any LUA script can call the `postmessage_frontend` and `postmessage_backend` functions on the [Engine Object](https://trisul.org/docs/lua/obj_engine.html) Any script, whether a backend or a frontend script can post to both types.
...
For details : See documentation of [postmessage_frontend](https://trisul.org/docs/lua/obj_engine.html#function_post_message_frontend) and [postmessage_backend](https://trisul.org/docs/lua/obj_engine.html#function_post_message_backend)
