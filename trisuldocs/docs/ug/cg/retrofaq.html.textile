...
bq. Powerful network analysis presented in a "Stack Overflow" style *tagged Q & A* format.
...
- The time selector := 
  ...
  =:
- The questions := 
  Simply click on a question to run the corresponding analysis for the interval selected.

  Click on _more_ if you want to load extra questions. Note that frequently used questions bubble up to the top of this list.
  =:
- Using the tags := 
  The small buttons besides each question is the tag. Click on a tag to load all questions with the same tag into the right hand side window called _Tagged Questions_
  =:


h2. Creating your own questions
...
bq. An example : If you type in @What is the bandwidth utilized by my server 10.10.200.10 ? #server #bandwidth@ 
...
h2. Managing existing questions
