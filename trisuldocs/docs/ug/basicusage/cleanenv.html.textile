h1. Cleaning the database

Sometimes you need to delete all the data and start over. 

h2. reset context 

Use @reset context <contextname>@ to delete all data. This method preserves all your configuration but only deletes the data. This command is available in both the trisulctl_hub and trisulctl_probe tools. 
...
You are not allowed to _reset context_ if the context is still running. If you get this error, use @stop context <contextname>@ and then a @reset@.
...
h3. Delete only specific probe data

p(autohint hand-o-right info). 
Use the _contextname@node_ syntax

If you only want to delete the Probe data ; i.e the raw packet capture and other probe specific data. You can say <code>reset context myctx1@probe0</code> This command will then only execute on probe0. 
...
*NOTE* You are not allowed to delete the @default@ context. Running @delete context default@ will get rid of the configuration and data for the default context, but the context itself will be available.  