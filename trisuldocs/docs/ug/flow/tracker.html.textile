...
A common use case of Flow Trackers is to track the so called 'Elephant flows'. Those are flows transferring a huge volume of data. But as you shall see below this is not the only type of tracker. Flow trackers are also necessary for you to use "flow tracker alerts":/docs/ug/alerts/ft.html. 

h2. Built in trackers
...
h3.  Configuring Custom Trackers

To configure flow trackers. 
%(hint command)Select Tools -> Flow Trackers -> Manage%


h3. Fill up the fields 
...
h4. Not on volume based trackers
...
%(hint command)Select Tools -> Flow Tracker%

h2.  Alerting on flow tracker activity  
