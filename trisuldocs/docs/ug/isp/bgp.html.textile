...
h2. Configuring Trisul as a BGP peer
...
- local_as := Your (ISPs) AS number. Important this is not an external ASN, because we are creating an I-BGP session.
- local_ip4 := IP Address of the Trisul-Probe 
- bgp_router_id := You can use the same value as @local_ip4@. An IP Address of the Trisul-Probe, this address will appear in BGP messages on the remote peers.
- neighbor := IP Address of the BGP neighbor then a space and the AS Number 

h3. Starting and verifying 
...
h2. Netflow vs BGP Peer address
...
* Login and go to _Netflow>Routers and Intefaces_  Note down the router IP address. This is the *Netflow exporter IP Address* say this is 10.17.17.20
...
# Make sure TCP Port 179 is open on the trisul probe @firewall-cmd --zone=public --add-port=179/tcp@
# There is a space , tab , or special character in the neighbor line of the config file
# Double check the softlinks
# Restart the probes 

h2. How to add a new IGW 

The procedure to add an IGW consists of two steps.

    # Configure NETSTREAM on the IGW to send  to either of the TWO VIP configured. 
    # Optionally configure BGP Route Receiver on the Trisul Probes and on the IGW.

h3. Configure NETSTREAM and BGP on IGW 
...
# Login to all probes
# Add the new IGW as  BGP Peer Open the file /usr/local/etc/trisul-probe/trisul_piranha.conf
# Add the peer as shown below at end of file 
