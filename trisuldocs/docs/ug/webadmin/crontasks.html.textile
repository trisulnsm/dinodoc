...
|_. Task Name|_.Run Frequency|_. Description|
...
h5. To view crontab list

Login as root, then type :

pre(language-bash). crontab -l
...
h5. To modify crontab list

Login as root, then type :

pre(language-bash). crontab -e 

h2. Background tasks log files 

div(info). 
Login as @admin@ user to manage menus.

All the Trisul background tasks send its output to the log file located at
  _**/usr/local/share/webtrisul/logs/back_tasks.log**_

You can also access these logs from the web interface

p(autohint hand-o-right info). 
Select _Webadmin_ -> Manage -> Web Server Logs ->Background tasks log
