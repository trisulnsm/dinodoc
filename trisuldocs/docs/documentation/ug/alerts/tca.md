...
| Keys                         |          | The key or key range within the *target counter group* on which you want to create a TCA. You may enter a single key or a key range<br></br> **- Single key** You can enter this in either human readable format (Example: Port-80, 192.168.1.33) or in Trisul key format: p-0050, C0.A8.00.01<br></br> **- Key range** Enter the first and last key in the range inclusive using a tilde *~* or *to*. Example IP range : `10.18.18.0 ~ 10.18.18.255` or port range (1 to 1024) `1 to 1024` using the *to* keyword<br></br>**- Plus Icon** Click the plus icon to select all keys. When dealing with a large number of keys (e.g., 3000 or more), manually selecting each key is impractical. The plus icon provides a convenient way to **select all** keys at once. You can then **deselect** specific keys to exclude them from the selection.                                            |
...
1) [**View individual alerts**](/docs/documentation/ug/alerts/tca#view-individual-alerts)
2) [**Filter Individual Alerts**](/docs/documentation/ug/alerts/tca#filter-individual-alerts) using the *Search Form* based on certain search criteria, filter individual alerts based on certain search criteria 
3) [**In-Depth Analysis of Individual Alerts**](/docs/documentation/ug/alerts/tca#in-depth-analysis-of-individual-alerts) using the *Drilldown* button
4) [**View Traffic Chart**](/docs/documentation/ug/alerts/tca#traffic-chart) with watermarks around the alert timestamp.
...
- **Download Button**: Click on the [download](/docs/documentation/ug/ui/elements#download-button) button at the top of the alerts to get the alert details downloaded in the form of PDF and XLSX. 
...
To facilitate targeted queries, a *search form* with [*Hide/Show Search Form*](/docs/documentation/ug/ui/elements#hide-show-search-form) option is provided, that allows you to specify a temporal range and a regular expression (*regex*) pattern to filter alerts.   
...
[**Time Frame**](/docs/documentation/ug/ui/elements#time-selector) Specify a temporal range to restrict search results to alerts generated within that timeframe.  
...
> You can create a Threshold Crossing Alerts(TCA) Module using [Module Templates](/docs/documentation/ug/ui/module_templates#alert-list) and add that module to any dashboard. And this module keeps auto updating itself as new TCA are generated. 


## Real Time Email Alerts and Scheduling

- You can configure email alert delivery for Threshold Crossing Alerts (TCA) using the [**Email Alerts Wizard**](/docs/documentation/ug/alerts/email_wizard).  
- You can [**Schedule a TCA Report**](/docs/documentation/ug/reports/schedreports) which will automatically email you a list of Threshold Crossing Alerts (TCAs) that got fired/cleared on a hourly or daily basis. The Report email is a single consolidated email that contains the details of all the TCAs. You can find the summary table of all the TCAs on the top of the email, clicking on a particular TCA on the summary table would quickly navigate you to the specified TCA detail without having to scroll through the page. And no email is sent if no TCAs are generated.
...
> Enabling [interface tracking](/docs/documentation/ug/netflow/interface_tracker) allows for richer TCA alerts.
