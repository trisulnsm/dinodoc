...
From the custom key monitor list, click on the **Action button** on the right side against the custom key monitor you want and click **Generate Report**. Select a time range from the [Time Selector](/docs/documentation/ug/ui/elements#time-selector) for the period which you want the report. This will generate a report in PDF format which can be downloaded.

### Schedule Report

Click on the **Schedule Report** from the action button menu to schedule a report periodically for the selected custom key monitor. Follow the steps as in the [**"Schedule a New Report"**](/docs/documentation/ug/reports/schedreports) doc and configure the report according to your requirements.
