...
Here we can see  we are using **6.2 MegaBytes** of disk per day using the [Storage Status](/docs/documentation/ag/admintasks/storage_status) tool
...
The retention period in number of days is specified in the [:memo: Trisul Hub Configuration](/docs/documentation/ref/trisulhubconfig) file. 
:::
...
You can also adjust the Oper and Ref , refer to the [Storage Architecture](/docs/documentation/ag/domain/storage_arch) document for details.

### Archiver type for large deployments

The Hub archiver (`trisul_archiver`) controls how slices move between storage tiers on disk. The algorithm is selected with `DBTasks/Archiver/Type` in [Trisul Hub Configuration](/docs/documentation/ref/trisulhubconfig):
...
For typical retention tuning (oper/ref/archive slice counts), `SLIDE` is sufficient. Switch to `RING` only when linear sliding cannot keep up with daily ingest volume. See the [Archiver](/docs/documentation/ref/trisulhubconfig#archiver) and [Extra ring archives](/docs/documentation/ref/trisulhubconfig#extra-ring-archives-ring-archiver) sections in the Hub configuration reference for XML examples.
...
The packet capture storage limits are defined in the [:memo: Trisul Probe Configuration](/docs/documentation/ref/trisulconfig) file in two parameters
...
If packet capture data needs to live on a different disk or mount point, relocate it using: [`trisulctl_probe relocate context`](/docs/documentation/ag/basictasks/reloc). This is commonly done when packet capture volumes grow large.
