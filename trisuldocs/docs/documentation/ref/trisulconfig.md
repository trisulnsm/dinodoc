...
Also see [trisulHubConfig.xml](/docs/documentation/ref/trisulhubconfig) for editing Hub node parameters 
...
- Each rule is an expression in the [Trisul filter format](/docs/documentation/ref/trisul_filter_format)
...
| MaxTCARangeAlerts        | 100      | When using TCA range alerts (see [TCA](/docs/documentation/ug/alerts/tca#tca-configuration) generate only these many alerts. The reason we need a safety cap on this feature is an incorrect configuration with a TCA range can result in uncontrolled alerts (eg when any IP crosses 1Kbps). For safety we have chosen a cap or 100             |
